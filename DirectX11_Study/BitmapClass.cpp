...
bool BitmapClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, int screenWidth, int screenHeight,
	const char* textureFilename, int bitmapWidth, int bitmapHeight)
{
	// 화면 크기를 멤버변수에 저장.
...
bool BitmapClass::Render(ID3D11DeviceContext* deviceContext, int positionX, int positionY)
{
	...
	// 그리기를 준비하기 위해 그래픽 파이프 라인에 꼭짓점과 인덱스 버퍼를 놓습니다.
...
bool BitmapClass::UpdateBuffers(ID3D11DeviceContext* deviceContext, int positionX, int positionY)
{
	...
	// 비트맵 왼쪽의 화면 좌표를 계산합니다.
	left = (float)((m_screenWidth / 2) * -1) + (float)positionX;

	// 비트맵 오른쪽의 화면 좌표를 계산합니다.
	right = left + (float)m_bitmapWidth;

	// 비트맵 상단의 화면 좌표를 계산합니다.
	top = (float)(m_screenHeight / 2) - (float)positionY;

	// 비트맵 아래쪽의 홤녀 좌표를 계산합니다.
